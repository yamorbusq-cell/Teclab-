...
function init(){
  ...
  form.addEventListener("submit", (e)=>{
    e.preventDefault();
    // recoger config
    const themeVal = document.getElementById("theme").value || "Tema 1";
    const subSel = document.getElementById("subtheme").value;
    const mode = document.querySelector('input[name="mode"]:checked').value;
    const orderOpt = document.querySelector('input[name="themeOrder"]:checked');
    const themeOrder = orderOpt ? orderOpt.value : "subthemes";

    // construir pool
    let pool = QUESTIONS.filter(q=>q.theme===themeVal);
    if (subSel && subSel !== "Todos"){ pool = pool.filter(q=>q.subtheme===subSel); }

    // fijar n
    let n;
    if (mode === "exam") n = 100;
    else if (subSel && subSel !== "Todos") n = 10;
    else if (subSel === "Todos" && themeOrder === "random") n = 50;
    else n = pool.length; // por subtemas: TODAS

    // ordenar/mezclar
    if (subSel && subSel !== "Todos"){
      pool = shuffle(pool).slice(0, n);
    } else {
      if (themeOrder === "subthemes"){
        pool = pool.slice().sort((a,b)=>{
          const sn = getSubthemeNumber(a.subtheme) - getSubthemeNumber(b.subtheme);
          if (sn !== 0) return sn;
          const aid = (typeof a.id === "number") ? a.id : parseInt((a.id||"0").toString().replace(/\D/g,"")||"0",10);
          const bid = (typeof b.id === "number") ? b.id : parseInt((b.id||"0").toString().replace(/\D/g,"")||"0",10);
          return aid - bid;
        });
      } else {
        pool = shuffle(pool).slice(0, n);
